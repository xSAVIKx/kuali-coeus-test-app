...
public class AwardDocumentRule extends org.kuali.kra.award.AwardDocumentRule {
	...
	protected boolean processCustomSaveDocumentBusinessRules(Document document) {
		...
		if (award.getExtension() != null
				&& ((AwardExtension) award.getExtension()).getAwardId() == null
				&& award.getAwardId() != null) {
			((AwardExtension) award.getExtension()).setAwardId(award
					.getAwardId());
		}

		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getAvcIndicator())) {
			((AwardExtension) award.getExtension())
					.setAvcIndicator(AVC_INDICATOR);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getA133Cluster())) {
			((AwardExtension) award.getExtension())
					.setA133Cluster(A133_CLUSTER);
		}
		if (((AwardExtension) award.getExtension())
				.getFringeNotAllowedIndicator() == null) {
			((AwardExtension) award.getExtension())
					.setFringeNotAllowedIndicator(FRINGE_NOT_ALLOWED);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getProgramIncome())) {
			((AwardExtension) award.getExtension())
					.setProgramIncome(PROGRAM_INCOME);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getStockAward())) {
		...
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getInterestEarned())) {
			((AwardExtension) award.getExtension())
					.setInterestEarned(INTEREST_EARNED);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getFederalRateDate())) {
			((AwardExtension) award.getExtension())
					.setFederalRateDate(FEDERAL_RATE_AGREEMENT_DATE);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getBuBmcFaSplit())) {
			((AwardExtension) award.getExtension())
					.setBuBmcFaSplit(BU_BMC_FA_SPLIT);
		}
		if (StringUtils.isEmpty(((AwardExtension) award.getExtension())
				.getForeignCurrencyAward())) {
			((AwardExtension) award.getExtension())
					.setForeignCurrencyAward(FOREIGN_CURRENCY_AWARD);
		}
	...
	private boolean skipRuleProcessing(Document document) {
		return AwardDocument.PLACEHOLDER_DOC_DESCRIPTION.equals(document
	 ...
	 *      <p/>
	 *      Run method in super class, then look for Sponsor Contact error and
	 *      remove it from AuditErrorMap
	 */
	@Override
	@Deprecated
	public boolean processRunAuditBusinessRules(Document document) {
		...
		AuditCluster contactAuditErrors = getAuditErrorMap().get(
				CONTACTS_AUDIT_ERRORS);

		if (contactAuditErrors != null) {
			Iterator<AuditError> iterator = contactAuditErrors
					.getAuditErrorList().iterator();
			while (iterator.hasNext()) {
				AuditError ver = iterator.next();
				String errorKey = ver.getErrorKey();
				if (StringUtils.equals(AWARD_SPONSOR_CONTACT_LIST_ERROR_KEY,
						errorKey)) {
					iterator.remove();
			...
			if (!contactAuditErrors.getAuditErrorList().isEmpty()) {
				getAuditErrorMap().put(CONTACTS_AUDIT_ERRORS,
						contactAuditErrors);
			}
		}
	...
	private boolean processChildTypeBusinessRule(MessageMap errorMap,
		...
		if (!isParentAward
				&& StringUtils.isBlank(((AwardExtension) award.getExtension())
						.getChildType())) {
			errorMap.putError("extension.childType",
					"error.award.childType.required", award.getAwardNumber());
			success = false;
		}
	...
	public boolean processFainRequiredBusinessRule(AwardDocument awardDocument) {
		...
		boolean isParentAward = award.getAwardNumber().endsWith("-00001")
				|| award.getAwardNumber().endsWith("-00000");
		if (isParentAward) {
			if (isFederalSponsor(award)) {
				if (isGrant(award) || isCooperativeAgreement(award)
						|| isSubGrant(award)) {
					if (StringUtils.isBlank(((AwardExtension) award
							.getExtension()).getFain())) {
						errorMap.putError(FAIN, FAIN_REQUIRED_FOR_AWARD,
	...
	private boolean processFainBusinessRule(AwardDocument awardDocument) {
		...
		String link = Constants.MAPPING_AWARD_HOME_PAGE + "."
				+ Constants.MAPPING_AWARD_HOME_DETAILS_AND_DATES_PAGE_ANCHOR;
		AuditCluster awardHomeAuditErrorsClustor = getAuditErrorMap().get(
				AWARD_HOME_AUDIT_ERRORS);
			...
			if (isFederalSponsor(award)) {
				if (isGrant(award) || isCooperativeAgreement(award)
					...
					if (StringUtils.isBlank(((AwardExtension) award
						...
						auditErrors.add(new AuditError(errorKey,
								FAIN_REQUIRED_FOR_AWARD, link));
						retVal = false;
					}
				}
			}
		...
		if (auditErrors.size() > 0) {
			getAuditErrorMap()
					.put(AWARD_HOME_AUDIT_ERRORS,
							new AuditCluster(
									Constants.MAPPING_AWARD_HOME_DETAILS_AND_DATES_PAGE_NAME,
	 ...
	 * @param awardDocument
	 */
	private void processPrimeSponsorAndAwardTypeSelectionBusinessRule(
			AwardDocument awardDocument) {
		Award award = awardDocument.getAward();
		String link = Constants.MAPPING_AWARD_HOME_PAGE + "."
				+ Constants.MAPPING_AWARD_HOME_DETAILS_AND_DATES_PAGE_ANCHOR;
		AuditCluster awardHomeAuditWarningsClustor = getAuditErrorMap().get(
				AWARD_HOME_AUDIT_WARNINGS);
		...
		if (isFederalAccount(award)) {
			if (StringUtils.isBlank(award.getPrimeSponsorCode())
					&& (isSubGrant(award) || isSubContract(award))) {
				String errorKey = AWARD_ERROR_PATH_PREFIX + PRIME_SPONSOR_CODE;
				auditWarnings.add(new AuditError(errorKey,
						PRIME_SPONSOR_REQUIRED_FOR_SELECTED_AWARD_TYPE_WARNING,
						link));
			}

			if (!StringUtils.isBlank(award.getPrimeSponsorCode())
					&& !(award.getPrimeSponsorCode().equals(award
							.getSponsorCode()))) {
				if (!(isSubGrant(award) || isSubContract(award))) {
					...
					auditWarnings
							.add(new AuditError(
									errorKey,
									PRIME_SPONSOR_PRESENT_WITH_INVALID_AWARD_TYPE_WARNING,
									link));
				}
	...
	protected boolean isCooperativeAgreement(Award award) {
		if (award.getAwardTypeCode() != null) {
			return AWARD_TYPE_CODE_COOPERATIVEAGREEMENT.equals(award
					.getAwardTypeCode());
		}
		return true;
	}
