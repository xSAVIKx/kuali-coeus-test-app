...
public class AwardTransmission extends KcPersistableBusinessObjectBase {
	...
	public String getAlldocumentNumbers() {
		...
		for (AwardTransmissionChild awardTransmissionChild : transmissionChildren) {
			documentNumbers += ", "
					+ makeDocumentOpenTag(awardTransmissionChild
							.getChildDocumentNumber());
		}
		return documentNumbers;
	}

	private String makeDocumentOpenTag(String documentNumber) {
		String workflowUrl = CoreApiServiceLocator
				.getKualiConfigurationService().getPropertyValueAsString(
						KRADConstants.WORKFLOW_URL_KEY);
		String url = String.format(DOC_HANDLER_URL_PATTERN, workflowUrl,
				documentNumber);
	...
	public Person getTransmitter() {
		transmitter = getPersonService().updatePersonIfNecessary(transmitterId,
				transmitter);
		return transmitter;
	}

	public Person getInitiator() {
		initiator = getPersonService().updatePersonIfNecessary(initiatorId,
				initiator);
		return initiator;
	}
	...
	public void setTransmissionChildren(
