...
public interface AwardHierarchyService {
    ...
    // Create new node, with Award copied from target node's Award. New node will be a root node and will be a branch node. (Descendant nodes are
    // copied)
    ...
    // Create new node, with Award copied from target node's Award. New node will be a child node of another node in SAME hierarchy and will be a
    // branch node
    ...
    // Create new node, with Award copied from target node's Award. New node will be a child node of another node in a DIFFERENT hierarchy and will be
    // a leaf node
    AwardHierarchy copyAwardAsChildOfAnAwardInAnotherHierarchy(AwardHierarchy sourceNode, AwardHierarchy targetParentNode);

    // Create new node, with Award copied from target node's Award. New node will be a child node of another node in same hierarchy and will be a leaf
    // node
    AwardHierarchy copyAwardAsChildOfAnAwardInCurrentHierarchy(AwardHierarchy sourceNode, AwardHierarchy targetParentNode);

    // Create new node, with Award copied from target node's Award. New node will be a root node and will be a leaf node. (Descendant nodes not
    // copied)
    ...
    public void populateAwardHierarchyNodes(Map<String, AwardHierarchy> awardHierarchyItems, Map<String, AwardHierarchyNode> awardHierarchyNodes,
            String currentAwardNumber, String currentSequenceNumber);

    public void populateAwardHierarchyNodesForTandMDoc(Map<String, AwardHierarchy> awardHierarchyItems,
            Map<String, AwardHierarchyNode> awardHierarchyNodes, String currentAwardNumber, String currentSequenceNumber, TimeAndMoneyDocument doc);

    public void createNodeMapsOnFormForSummaryPanel(Map<String, AwardHierarchyNode> awardHierarchyNodes, Map<String, String> previousNodeMap,
            Map<String, String> nextNodeMap);
