...
/**
 * <p>
 * A service implementation which provides integration with Boston University's SAP implementation. Provides two primary operations, one for
 * performing validation against incoming award hierarchy data, and other for transmitting award hierarchy data to SAP.
 * <p/>
 * <p>
...
public class SapIntegrationServiceImpl implements SapIntegrationService, InitializingBean {
    ...
    protected SIKCRMPROCESSOUTBOUND newWebServicePort(PrintWriter sendWriter, PrintWriter receiveWriter) {
        ...
        if (StringUtils.isBlank(sapWebServiceUrlValue)) {
            throw new IllegalStateException("The " + SAP_SERVICE_WSDL_URL_PARAM
            ...
            if (grantMessages != null && grantMessages.getItem() != null) {
                for (ZGRANTMESSAGES grantMessage : grantMessages.getItem()) {
                    if (ERROR_MESSAGE_TYPE.equals(grantMessage.getTYPE())) {
                        isSuccess = false;
                        failureMessage
                                .append("    [Grant Failure Message: " + grantMessage.getGRANTNBR() + "] - " + grantMessage.getMESSAGE() + "\n");
                    } else {
                        hasSuccessMessage = true;
                        successMessage
                                .append("    [Grant Success Message: " + grantMessage.getGRANTNBR() + "] - " + grantMessage.getMESSAGE() + "\n");
                    }
                }
            }
            ...
            if (sponsoredProgramsMessages != null && sponsoredProgramsMessages.getItem() != null) {
                for (ZSPPROGRAMMESSAGES sponsoredProgramsMessage : sponsoredProgramsMessages.getItem()) {
                    if (ERROR_MESSAGE_TYPE.equals(sponsoredProgramsMessage.getTYPE())) {
                        isSuccess = false;
                        failureMessage.append("    [Sponsored Program Failure Message: " + sponsoredProgramsMessage.getSPONSOREDPROG() + "] - "
                                + sponsoredProgramsMessage.getMESSAGE() + "\n");
                    } else {
                        hasSuccessMessage = true;
                        successMessage.append("    [Sponsored Program Success Message: " + sponsoredProgramsMessage.getSPONSOREDPROG() + "] - "
                                + sponsoredProgramsMessage.getMESSAGE() + "\n");
                    }
                }
            }
    ...
    protected ZGMGRANTSTRUCTURE constructGrant(Award award) {
        ...
        // Position 9
        grantHeader.setVALIDFROM(dateToString(DateUtils.convertToSqlDate(awardAmountInfo.getCurrentFundEffectiveDate())));

        // BU Customization ID: N/A mkousheh 20110620 - Set Valid From Budget to
        // 3 months if null
        if (awardAmountInfo.getCurrentFundEffectiveDate() != null) {
            java.util.Date tmpDate = org.apache.commons.lang3.time.DateUtils.addMonths(awardAmountInfo.getCurrentFundEffectiveDate(), -3);
            grantHeader.setVALIDFROMBUDGET(dateToString(DateUtils.convertToSqlDate(tmpDate)));
        }

        // Position 10
        grantHeader.setVALIDTO(dateToString(DateUtils.convertToSqlDate(awardAmountInfo.getObligationExpirationDate())));
        // BU Customization ID: N/A mkousheh 20110620 - Set Valid To Budget to 1
        // year if null
        if (awardAmountInfo.getObligationExpirationDate() != null) {
            java.util.Date tmpDate2 = org.apache.commons.lang3.time.DateUtils.addYears(awardAmountInfo.getObligationExpirationDate(), 1);
     ...
     * <p/>
     * When Frequency_Code = 1, Invoice_Frequency = "M� When Frequency_Code = 2, Invoice_Frequency = "Q" All other values for Frequency_Code,
     * Invoice_Frequency = "O"
     *
     * @param frequencyCode
     ...
     * <p/>
     * When Report_Code = 1, Invoice_Form = "ZFI_GM_SF425" When Report_Code = 3, Invoice_Form = "ZFI_GM_SF270" When Report_Code = 25, Invoice_Form =
     * "ZFI_GM_SF1034-1035" When Report_Code = 26, Invoice_Form = "ZFI_GM_BU-Standard" When Report_Code = 28, Invoice_Form = "Manual"
     */
    private String convertReportCodeToInvoiceForm(String reportCode) {
        String invoiceForm = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.REPORT_CODE_TO_INVOICE_FORM_MAPPING,
                reportCode);
     ...
     * <p/>
     * When a row exists on AWARD_REPORT_TERMS where AWARD_REPORT_TERMS.REPORT_CLASS_CODE = 8 and AWARD_REPORT_TERMS.REPORT_CODE = 27 � Send an "X"
     * value for Advance_Payment. Otherwise don"t send a value.
     *
     * @param award
     ...
     * following mapping:
     * <p/>
     * If an Award has AWARD_SPONSOR_TERM_ID = 420; Property_Owner_Title = "BU" If an Award has AWARD_SPONSOR_TERM_ID = 421; Property_Owner_Title =
     * "FD" If an Award has AWARD_SPONSOR_TERM_ID = 422; Property_Owner_Title = "OI" Otherwise, don"t send anything
     *
     * @param award
     ...
     * <p/>
     * If an Award has a row on the AWARD_COST_SHARE where Cost_Share_Type_Code = 3, then Cost_Share_Memo_Match = "X". Otherwise don't send a value.
     *
     * @param award
    ...
    private String convertAwardTypeCodeToAwardType(int awardTypeCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.AWARD_TYPE_MAPPING,
                Integer.toString(awardTypeCode));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert award type, given award type value was not understood: " + awardTypeCode
     ...
     * <p/>
     * <p>
     * Note that if an award has multiple sponsored terms, they all should be sent (not only the most recent).
     */
    ...
    protected ZGMSPPROGRAMSTRUCTURE constructSponsoredProgram(Award award, boolean costSharing) {
        ...
        if (!awardBudgetVersionToBePosted) {
            ...
            BigDecimal fAndARate = calculateApplicableFandARate(award);
            if (fAndARate != null
                    && (((AwardExtension) award.getExtension()).getFederalRateDate() == null || "NA".equalsIgnoreCase(((AwardExtension) award
                ...
                overheadKey = getParameterService().getSubParameterValueAsString(AwardDocument.class,
                        BUConstants.FEDERAL_RATE_DATE_OVERHEAD_KEY_FIELD_MAPPINGS, ((AwardExtension) award.getExtension()).getFederalRateDate());
        ...
        if (costSharing) {
            ...
            if (tmpAwardCostShare != null && ((AwardExtension) award.getExtension()).getLastTransmissionDate() != null
                    && tmpAwardCostShare.getUpdateTimestamp() != null
                    && ((AwardExtension) award.getExtension()).getLastTransmissionDate().before(tmpAwardCostShare.getUpdateTimestamp())) {
        ...
        if (!costSharing && !hierarchy.hasChildren()) {
            ...
            if (!awardBudgetVersionToBePosted) {
                ...
                // }
                // }
                AwardTransmissionChild lastTransmittedChild = getLatestChildAwardTransmitted(hierarchy.getRoot().getAward(), award.getAwardNumber(),
                        true);
                if (lastTransmittedChild != null) {
     ...
     * according to the following mapping:
     * <p/>
     * If "Standard�; Child_Type = ST1 If "Fabrication", Child_Type = P01 If "Participant Support Costs", Child_Type = PS1 If "Subaward", Child_Type =
     * SA1
     *
     * @param childType
    ...
    private String convertChildType(String childType, Award award) {
        ...
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert child type, given child type value was not understood: " + childType
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    protected ZGMSPONSORSTRUCTURE constructSponsorFromOrganization(Award award, Organization organization) {
        ...
        if (sponsor == null) {
            throw new IllegalStateException("Failed to locate a Sponsor record for the given organization with id: "
    ...
    protected ZGMSPONSORSTRUCTURE constructSponsorFromBillingPartner(Award award, AwardSponsorContact billingPartner) {
        ...
        if (rolodex == null) {
            ...
            if (rolodex == null) {
                throw new IllegalStateException("Billing partner does not have a rolodex entry.  Award contact id of billing partner is: "
                        + billingPartner.getAwardContactId() + ", rolodex id is: " + billingPartner.getRolodexId());
            }
        }
    ...
    private void generateProgramGroupChildren(SPONSOREDPROGRAMSGRP programGroups, AwardHierarchy awardHierarchy) {
        for (AwardHierarchy childHierarchy : awardHierarchy.getChildren()) {
            ...
            if ("Group".equalsIgnoreCase(((AwardExtension) (childHierarchy.getAward().getExtension())).getChildType())
     ...
     * responses.
     * <p/>
     * This allows us to implement section 1.6 of the functional specification and deal with the cases where an award is being assigned a new
     * sponsored program id from SAP.
     * <p/>
     ...
     */
    private Map<Long, String> extractSponsoredProgramIds(List<Long> interfacedSponsoredProgramIds, SapTransmission transmission,
        ...
        if (transmission.getAward() == null) {
            ...
            if (transmission.getAward().getAwardTransactionTypeCode().equals(AWARD_TRANSACTION_TYPE_NO_COST_EXTENSION)
                ...
                for (Award childAward : transmission.getChildAwards()) {
                    ...
                    if (awardBudgetVersionToBePosted) {
                        ErrorMessageKeys.PARENT_TRANSACTION_TYPE_NOCOST_EXT_OR_ADMIN_CHANGE.populate(transmission.getAward().getAwardId(),
                                validationResults, transmission.getAward().getAwardTransactionType().getDescription(), childAward.getAwardNumber(),
                                transmission.getAward().getSequenceNumber().toString());
                        break;
                    }
                }
            }
            ...
            if (transmission.getAward().getAwardAmountInfos().size() > 0) {
                ...
                try {
                    if (StringUtils.isNotBlank(timeAndMoneyDocumentNumber)) {
                        TimeAndMoneyDocument timeAndMoneyDocument = (TimeAndMoneyDocument) getDocumentService().getByDocumentHeaderId(
                                timeAndMoneyDocumentNumber);
                        if (timeAndMoneyDocument != null && timeAndMoneyDocument.getDocumentHeader().hasWorkflowDocument()
    ...
    private AwardTransmission getLatestAwardTransmission(Award rootAward, boolean getLastSuccesful) {
        ...
        if (getLastSuccesful) {
            ...
            if (((AwardExtension) rootAward.getExtension()).getAwardTransmissions() != null
                    && ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() > 0)
                latestTransmission = ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().get(
                        ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() - 1);
        }
    ...
    private void checkPrimaryAwardForChanges(Award award, ValidationResults validationResults) {
        ...
        if (award != null && lastTransmission != null && lastTransmission.isSuccess()) {
            if (award.getAccountTypeCode() != null && lastTransmission.getAccountTypeCode() != null
                ...
                ErrorMessageKeys.PARENT_ACCOUNT_TYPE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmission.getAccountTypeCode().toString(), award.getAccountTypeCode().toString());
            }
            ...
            if ((RRB_BASIS_OF_PAYMENT.equals(award.getBasisOfPaymentCode()) && MILESTONE_BASIS_OF_PAYMENT.equals(lastTransmission
                    .getBasisOfPaymentCode()))
                    || (MILESTONE_BASIS_OF_PAYMENT.equals(award.getBasisOfPaymentCode()) && RRB_BASIS_OF_PAYMENT.equals(lastTransmission
                            .getBasisOfPaymentCode()))) {
                ErrorMessageKeys.PARENT_PAYMENT_BASIS_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmission.getBasisOfPaymentCode(), award.getBasisOfPaymentCode());
            }
            ...
            if (DHHS_LOC_MATHOD_OF_PAYMENT.equalsIgnoreCase(lastTransmission.getMethodOfPaymentCode())
                    && !StringUtils.equals(award.getMethodOfPaymentCode(), lastTransmission.getMethodOfPaymentCode())) {
                ErrorMessageKeys.PARENT_PAYMENT_METHOD_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmission.getMethodOfPaymentCode(), award.getMethodOfPaymentCode());
            }
            if (!StringUtils.equals(award.getSponsorCode(), lastTransmission.getSponsorCode())) {
                ErrorMessageKeys.PARENT_SPONSOR_CODE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmission.getSponsorCode(), award.getSponsorCode());
            }
        }
    }
    ...
    private void checkChildAwardForChanges(Award primaryAward, Award award, ValidationResults validationResults) {
        ...
        if (award != null && lastTransmittedChild != null) {
            if (!"group".equalsIgnoreCase(((AwardExtension) award.getExtension()).getChildType())
                    && !StringUtils.equals(((AwardExtension) award.getExtension()).getChildType(), lastTransmittedChild.getChildType())) {
                ErrorMessageKeys.CHILD_CHILD_TYPE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmittedChild.getChildType(), ((AwardExtension) award.getExtension()).getChildType());
            }
            if (!StringUtils.equals(award.getLeadUnitNumber(), lastTransmittedChild.getLeadUnitNumber())) {
                ErrorMessageKeys.CHILD_LEAD_UNIT_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(),
                        lastTransmittedChild.getLeadUnitNumber(), award.getLeadUnitNumber());
            }
        }
    }
    ...
    private void validateAwardIsFinal(ValidationResults results, Award award) {
        ...
        try {
            VersionHistory versionHistory = vhs.findPendingVersion(Award.class, award.getAwardNumber(),
            ...
            } else if (award.getAwardDocument().getDocumentHeader().hasWorkflowDocument()
                    && !award.getAwardDocument().getDocumentHeader().getWorkflowDocument().isFinal()) {
    ...
    private boolean isLocMethodOfPayment(String methodOfPaymentCode) {
        String locMethodOfPaymentList = parameterService.getParameterValueAsString(AwardDocument.class, BUConstants.BU_LOC_METHOD_OF_PAYMENT_CD);
     ...
     * from an Award. This code was largely reverse engineered by looking at {@link AwardHierarchyUIServiceImpl}.
     *
     * @param award
    ...
    private AwardHierarchyNode getAwardHierarchyNode(Award award) {
        ...
        Map<String, AwardHierarchy> awardHierarchyItems = getAwardHierarchyService().getAwardHierarchy(award.getAwardNumber(),
                new ArrayList<String>());
        getAwardHierarchyService().populateAwardHierarchyNodes(awardHierarchyItems, awardHierarchyNodes, award.getAwardNumber(),
    ...
    private String convertAccountTypeToGrantType(int accountType, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.ACCOUNT_TYPE_MAPPING,
                Integer.toString(accountType));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert account type, given account type value was not understood: " + accountType
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertActivityTypeToFunctionalArea(String activityType, Award award) {
        ...
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert activity type, given activity type value was not understood: " + activityType
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertBasisOfPaymentToBillingRule(String basisOfPaymentCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.BASIS_OF_PAYMENT_MAPPING,
                basisOfPaymentCode);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert basis of payment code, given basis of payment value was not understood: "
                    + basisOfPaymentCode + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertMethodOfPaymentToLetterOfCredit(String methodOfPaymentCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.METHOD_OF_PAYMENT_MAPPING,
                methodOfPaymentCode);
    ...
    private String convertProjectRoleToResponsibility(String projectRole, Award award) {
        ...
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert project role, given project role value was not understood: " + projectRole
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertStatusCodeToResponsibility(int statusCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.STATUS_CODE_MAPPING,
                Integer.toString(statusCode));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert status code, given status code value was not understood: " + statusCode
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertSponsorCodeToSponsorType(String sponsorCode, Award award) {
        ...
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert sponsor code, given sponsor code value was not understood: " + sponsorCode
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private String convertSponsorTermToSponsorClass(long sponsorTerm, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.SPONSOR_TERM_MAPPING,
                Long.toString(sponsorTerm));
        if (StringUtils.isEmpty(returnValue)) {
            LOG.info("[awardId: " + award.getAwardId() + ", awardNumber: " + award.getAwardNumber() + "] - "
                    + "Failed to convert sponsor term, given sponsor term value was not understood: " + sponsorTerm);
        }
        return returnValue;
    }
    ...
    private String convertLeadUnitToBusinessArea(String leadUnitNumber, Award award) {
        ...
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert lead unit, given lead unit value was not understood: " + leadUnitNumber
                    + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
