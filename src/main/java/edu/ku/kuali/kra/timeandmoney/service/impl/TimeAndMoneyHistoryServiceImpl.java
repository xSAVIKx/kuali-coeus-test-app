...
public class TimeAndMoneyHistoryServiceImpl extends org.kuali.kra.timeandmoney.service.impl.TimeAndMoneyHistoryServiceImpl implements
edu.ku.kuali.kra.timeandmoney.service.TimeAndMoneyHistoryService {
    ...
    public void getTimeAndMoneyHistory(String awardNumber, Map<Object, Object> timeAndMoneyHistory, List<Integer> columnSpan)
        ...
        timeAndMoneyHistory.put(buildForwardUrl(award.getAwardDocument().getDocumentNumber()), award.getAwardDescriptionLine());
                            ...
                            if (awardAmountInfo.getTimeAndMoneyDocumentNumber().equals(doc.getDocumentNumber())) {
                                if (StringUtils.equalsIgnoreCase(doc.getDocumentNumber(), awardAmountInfo.getTimeAndMoneyDocumentNumber())) {
                                    if (awardAmountTransactions.size() > 0) {
                                        ...
                                        timeAndMoneyHistory.put(buildForwardUrl(doc.getDocumentNumber()), award.getAwardDescriptionLine());
                                    }
                                    ...
                                    List<TransactionDetail> transactionDetailsA = ((List<TransactionDetail>) businessObjectService
                                            .findMatchingOrderBy(TransactionDetail.class, fieldValues5, "sourceAwardNumber", true));
                        ...
                        if (!(awardAmountInfo.getTimeAndMoneyDocumentNumber() == null)) {
                            if (awardAmountInfo.getTimeAndMoneyDocumentNumber().equals(doc.getDocumentNumber())) {
                                if (StringUtils.equalsIgnoreCase(doc.getDocumentNumber(), awardAmountInfo.getTimeAndMoneyDocumentNumber())) {
                                    ...
                                    List<TransactionDetail> transactionDetailsA = ((List<TransactionDetail>) businessObjectService
                                            .findMatchingOrderBy(TransactionDetail.class, fieldValues3, "sourceAwardNumber", true));
                                    List<TransactionDetail> transactionDetailsB = ((List<TransactionDetail>) businessObjectService
                                            .findMatchingOrderBy(TransactionDetail.class, fieldValues3a, "sourceAwardNumber", true));
                        ...
                        if (StringUtils.equalsIgnoreCase(doc.getDocumentNumber(), awardAmountInfo.getTimeAndMoneyDocumentNumber())) {
                            if (awardAmountInfo.getTransactionId() == null) {
                                ...
                                List<TransactionDetail> dateTransactionDetails = ((List<TransactionDetail>) businessObjectService
                                        .findMatchingOrderBy(TransactionDetail.class, fieldValues4, "sourceAwardNumber", true));
                                int i = 0;
    ...
    @Override
    @SuppressWarnings("unchecked")
    public void buildTimeAndMoneyHistoryObjects(String awardNumber, List<AwardVersionHistory> awardVersionHistoryCollection, boolean doNothing)
            throws WorkflowException {
        List<Award> awardVersionList = (List<Award>) businessObjectService.findMatchingOrderBy(Award.class, getHashMapToFindActiveAward(awardNumber),
                "sequenceNumber", true);
        ...
        docs = (List<TimeAndMoneyDocument>) businessObjectService.findMatchingOrderBy(TimeAndMoneyDocument.class, fieldValues1, "documentNumber",
                true);
        ...
        for (Award award : awardVersionList) {
            ...
            awardVersionHistory.setTimeAndMoneyDocumentHistoryList(getDocHistoryAndValidInfosAssociatedWithAwardVersion(docs,
                    award.getAwardAmountInfos(), award));

            awardVersionHistoryCollection.add(awardVersionHistory);
        }
    }

    public void buildTimeAndMoneyHistoryObjects(String awardNumber, List<org.kuali.kra.timeandmoney.AwardVersionHistory> awardVersionHistoryCollection)
            throws WorkflowException {
        List<Award> awardVersionList = (List<Award>) businessObjectService.findMatchingOrderBy(Award.class, getHashMapToFindActiveAward(awardNumber),
                "sequenceNumber", true);
        // we want the list in reverse chronological order.
        ...
        removeCanceledDocs(docs);
        for (Award award : awardVersionList) {
            org.kuali.kra.timeandmoney.AwardVersionHistory awardVersionHistory = new org.kuali.kra.timeandmoney.AwardVersionHistory(award);
            awardVersionHistory.setDocumentUrl(buildForwardUrl(award.getAwardDocument().getDocumentNumber()));
            awardVersionHistory.setAwardDescriptionLine(award.getAwardDescriptionLine());
            awardVersionHistory.setTimeAndMoneyDocumentHistoryList(getDocHistoryAndValidInfosAssociatedWithAwardVersion(docs,
                    award.getAwardAmountInfos(), award));
        ...
        for (AwardAmountInfo awardAmountInfo : validInfos) {
            if (!(awardAmountInfo.getTimeAndMoneyDocumentNumber() == null)) {
                if (StringUtils.equalsIgnoreCase(doc.getDocumentNumber(), awardAmountInfo.getTimeAndMoneyDocumentNumber())) {
                    ...
                    List<TransactionDetail> transactionDetails = ((List<TransactionDetail>) businessObjectService.findMatchingOrderBy(
                            TransactionDetail.class, fieldValues1, "transactionDetailId", true));
                    List<TransactionDetail> transactionDetailsA = ((List<TransactionDetail>) businessObjectService.findMatchingOrderBy(
                            TransactionDetail.class, fieldValues1a, "transactionDetailId", true));
                    ...
                    fieldValues3.put("transactionId", awardAmountInfo.getTransactionId());
                    List<TransactionDetail> transactionDetailsOnParent = ((List<TransactionDetail>) businessObjectService.findMatchingOrderBy(
                            TransactionDetail.class, fieldValues3, "transactionDetailId", true));
                    ...
                    transactionDetails.addAll(transactionDetailsA);
                    List<TransactionDetail> transactionDetailsB = ((List<TransactionDetail>) businessObjectService.findMatchingOrderBy(
                            TransactionDetail.class, fieldValues2, "transactionDetailId", true));
            ...
            if (!(awardAmountInfo.getTimeAndMoneyDocumentNumber() == null)) {
                if (StringUtils.equalsIgnoreCase(doc.getDocumentNumber(), awardAmountInfo.getTimeAndMoneyDocumentNumber())) {
                    if (awardAmountInfo.getTransactionId() == null) {
                        ...
                        List<TransactionDetail> dateTransactionDetails = ((List<TransactionDetail>) businessObjectService.findMatchingOrderBy(
                                TransactionDetail.class, fieldValues, "sourceAwardNumber", true));
    ...
    public List<TimeAndMoneyDocument> buildTimeAndMoneyListForAwardDisplay(Award award) throws WorkflowException {
        ...
        List<TimeAndMoneyDocument> docs = (List<TimeAndMoneyDocument>) businessObjectService.findMatchingOrderBy(TimeAndMoneyDocument.class,
    ...
    public List<TimeAndMoneyDocumentHistory> getDocHistoryAndValidInfosAssociatedWithAwardVersion(List<TimeAndMoneyDocument> docs,
        ...
        for (TimeAndMoneyDocument doc : awardVersionDocs) {
            ...
            docHistory.setTimeAndMoneyDocumentDescriptionLine(buildNewTimeAndMoneyDescriptionLine(doc, awardAmountInfos));
    ...
    protected String buildNewTimeAndMoneyDescriptionLine(TimeAndMoneyDocument doc, List<AwardAmountInfo> awardAmountInfos) {
        ...
        return "Time And Money Document: " + transactionTypeDescription + ", Award Number : " + originatingAwardNumber + " - Award Version : "
        + originatingAwardVersion + ", notice date: " + noticeDate + ", updated " + getUpdateTimeAndUser(doc) + ". Comments: "
